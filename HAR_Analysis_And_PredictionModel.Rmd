...
Human Activity Recognition. Analysis & Prediction Model
============================
...
We reduced number of predictors to `r ncol(training)-1`. Let's now fit a model. We will use `{caret}` package for this purpose. We chose a random forest model at first and look at its accuracy. When building a model, we will also use 10 fold cross validation by passing `trainControl(method='cv')` function when training the model. This will allow us to find the best fit model for our training set. Note: it might take a significant amount of time to fit the model. Look into _parallel_ package for possible speed up
...
We get pretty good accuracy rate __0.9993%__. We can see that 500 trees were built with 40 variables tried at each split. Let's now predict the values for our remaining chunk of the dataset `testing` and analyze __out of sample error rate__
...
As seen from above, out of sample error rate is very low. Let's now predict value for a given validation dataset and prepare files for submission
